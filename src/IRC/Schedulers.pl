...
sub setupSchedulers {
    ...
    &apacheannounce(1);	  #blat #apache with a message
    &uptimeLoop(1);
    &logLoop(1);
    &chanlimitCheck(1);
...
sub apacheannounce {
    if (@_) {
        &ScheduleThis( 900, 'apacheannounce' );    
        return if ( $_[0] eq '2' );              # defer.
    }
    open (ASFCHANNELS, "/home/fajita/fajita/files/infobot.asfchannels") || die "We couldn't open the file!";

    my $channel = "#apache";
    my $message = <ASFCHANNELS>;
    close (ASFCHANNELS);
    &msg( $channel, $message );
