...
sub scan(&$$) {
  ...
  if($message=~/^(\w+)\susage$/ && &::IsFlag("d")) {
    #print "We matched!\n";
    $searchstring=lc($1);
    $searchstring=~ s/([^A-Za-z0-9])/sprintf("%%%02X", ord($1))/seg;
    my $ua = new LWP::UserAgent;
    $ua->timeout(10);
    my $request = new HTTP::Request('GET', "http://arreyder.com:8983/solr/select?q=$searchstring&qt=dismaxusage");
    ...
    my $nodeset = $xpath->find("/response/result/doc/arr[\@name='ds-usage']/str");
    ...
    $reply = "$searchstring usage: ".$reply." |\n";
    $callback->($reply);
   return 1;
  }
