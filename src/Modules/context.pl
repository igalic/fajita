...
sub scan(&$$) {
  ...
  if($message=~/^(\w+)\scontext$/ && &::IsFlag("d")) {
    #print "We matched!\n";
    $searchstring=lc($1);
    $searchstring=~ s/([^A-Za-z0-9])/sprintf("%%%02X", ord($1))/seg;
    my $ua = new LWP::UserAgent;
    $ua->timeout(10);
    my $request = new HTTP::Request('GET', "http://arreyder.com:9080/docsearch/select?q=$searchstring&df=ds-name&fl=ds-name+ds-context&rows=1");
    ...
    my $nodeset = $xpath->find("/response/result/doc/arr[\@name='ds-context']/str");
    ...
    $reply = "$searchstring may be used in these contexts: ".$reply." |\n";
    $callback->($reply);
